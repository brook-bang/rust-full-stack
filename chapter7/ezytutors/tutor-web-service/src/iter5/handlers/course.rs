...
/*  复制粘贴命令
curl.exe -X POST http://localhost:3000/courses/ -H "Content-Type: application/json" -d "{\"course_id\":4, \"tutor_id\":1, \"course_name\":\"This is the fourth course\"}"
*/
pub async fn post_new_course(
    new_course: web::Json<CreateCourse>,
    app_state: web::Data<AppState>,
) -> Result<HttpResponse, EzyTutorError> {
    post_new_course_db(&app_state.db, new_course.0)
        .await
        .map(|course| HttpResponse::Ok().json(course))
}
